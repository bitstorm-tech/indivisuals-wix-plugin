...
const getHandleStyle = (type: string) => {
  ...
  switch (type) {
    ...
    case 'n':
      return `${baseStyle} -top-1 left-1/2 -translate-x-1/2 cursor-n-resize`;
    case 's':
      return `${baseStyle} -bottom-1 left-1/2 -translate-x-1/2 cursor-s-resize`;
    case 'e':
      return `${baseStyle} -right-1 top-1/2 -translate-y-1/2 cursor-e-resize`;
    case 'w':
      return `${baseStyle} -left-1 top-1/2 -translate-y-1/2 cursor-w-resize`;
    default:
      return baseStyle;
  }
};
